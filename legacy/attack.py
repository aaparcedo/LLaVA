...
class Attack(object):
    ...
    def _check_inputs(self, images):
        ...
        print(f'images dtype in check inputs in attack.py: {images.dtype}')
        return images

    def _check_outputs(self, images):
        if self._normalization_applied:
            images = self.normalize(images)
        print(f'images dtype in check outputs in attack.py: {images.dtype}')
        return images
    ...
    # Alejandro Aparcedo
    # Refactored to work with CLIPVisionModelWithProjection()
    def get_logits_clip(self, inputs, text_embeds, classes):
        if self._normalization_applied:
            inputs = self.normalize(inputs)
        ...
        image_embeds = vision_model(inputs).image_embeds
    ...
    def set_normalization_used(self, mean, std):
        ...
        mean = torch.tensor(mean).reshape(1, n_channels, 1, 1).half()
        # std = torch.tensor(std).reshape(1, n_channels, 1, 1)
        std = torch.tensor(std).reshape(1, n_channels, 1, 1).half()
    ...
    def load(load_path, batch_size=128, shuffle=False, normalize=None,
        ...
        if save_dict['save_type'] == 'int':
            ...
            save_dict['adv_inputs'] = save_dict['adv_inputs'].half()/255

            if load_clean_inputs:
                # save_dict['clean_inputs'] = save_dict['clean_inputs'].float() / 255  # nopep8
                save_dict['clean_inputs'] = save_dict['clean_inputs'].half() / 255  # nopep8


        if normalize is not None:
            n_channels = len(normalize['mean'])
            # mean = torch.tensor(normalize['mean']).reshape(1, n_channels, 1, 1)
            mean = torch.tensor(normalize['mean']).reshape(1, n_channels, 1, 1).half()
            # std = torch.tensor(normalize['std']).reshape(1, n_channels, 1, 1)
            std = torch.tensor(normalize['std']).reshape(1, n_channels, 1, 1).half()
