...
def eval_model(args):
	...
	print(line)
        ...
        # print(f'Prompt for question id {idx}: {prompt}', flush=True)
        ...
        if args.data_augmentation is not None:
            image = apply_data_augmentation(image, args.data_augmentation)
        ...
        if args.conv_mode == 'simple':
            while True:
                ...
                for pattern in ['###', 'Assistant:', 'Response:']: # added the "Human:" here, attempting to remove the "human:" from the sample vqa answers
        ...
        # print('Full conversation for prompt')
        # print(conv.messages, flush=True)



        # Append outputs to the original question's 'text' field
        line['text'] += " " + outputs
    ...
    with open(os.path.expanduser(args.question_file), "w") as f:
