...
class LLAVA2Dataset(CLIPDataset):
    def __init__(self, 
        ...
        *** Dataset for LLAVA V1.5.
        ...
        self.query_input_ids = None if not hasattr(model.args, 'query') else text_to_input_ids(self.model.args.query, self.model)

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        image, base_path = self._load_image(idx)
        image_tensor = process_images([image], self.model.image_processor, self.model.model.config)[0]
...
class BLIP2Dataset(CLIPDataset):
    def __init__(self, 
                dataset = 'coco',
                task = 'classification',
                image_folder = None,
        ...
        *** Dataset for LLAVA V1.5.
        ...
        super().__init__(dataset=dataset, task=task, 
                         ...
                         model=model)
        self.model = model
        self.query_input_ids = None if not hasattr(model.args, 'query') \
            else model.processor(text=model.args.query, padding=True, truncation=True, return_tensors="pt")['input_ids'].cuda()

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        image, base_path = self._load_image(idx)
        image_tensor = self.model.processor(images=image, return_tensors='pt')['pixel_values'][0].cuda().half()
        ...
        if self.task == 'classification':
            ...
            if self.task == 'vqa':
                ...
                input_ids = self.model.processor(text=qs, padding=True, truncation=True, return_tensors="pt")['input_ids'][0].cuda()
                return input_ids, image_tensor, self.data_list[idx]["question_id"]
...
def get_dataloader(args, model):    
    if 'llava-v1.5' in args.model_path:
            dataset = LLAVA2Dataset(dataset=args.dataset, model=model, data_file=args.data_file, image_folder=args.image_folder, task=args.task, subset=args.subset)
    elif 'llava' in args.model_path or 'clip' in args.model_path:
        dataset = CLIPDataset(dataset=args.dataset, model=model, data_file=args.data_file, image_folder=args.image_folder, task=args.task, subset=args.subset)
    elif 'blip2' in args.model_path:
        dataset = BLIP2Dataset(dataset=args.dataset, model=model, data_file=args.data_file, image_folder=args.image_folder, task=args.task, subset=args.subset)
        
    dataloader = DataLoader(dataset, batch_size=args.batch_size, num_workers=0, shuffle=False, pin_memory=False)
