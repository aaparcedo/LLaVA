...
from llava.mm_utils import text_to_input_ids
...
def collate_fn(batch):
    return batch[0][0].unsqueeze(0), batch[0][1], batch[0][2]
...
def encode_both(args, model, dataloader):
    with torch.no_grad():
        ...
        for images, text, _ in tqdm(dataloader):
            ...
            if 'clip' in args.model_path:
            ...
            elif 'blip2' in args.model_path and 'instruct' not in args.model_path:
                ...
                if "llava" in args.model_path:
                    input_ids = text_to_input_ids(args.prompt, model).unsqueeze(0)
                    response = model.generate(input_ids=input_ids, image=images)
                else:
                    response = model.generate(args.prompt, images)
                input_ids = clip_text_tokenizer(response, return_tensors="pt", padding=True).input_ids.cuda()
                input_encoding = clip_text_encoder(input_ids).text_embeds.cuda()
                input_encodings.append(input_encoding)
            
                label_input_ids = clip_text_tokenizer(text, return_tensors="pt", padding=True).input_ids.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image-folder", type=str, default="/groups/sernam/datasets/coco/val2014")
    parser.add_argument("--data-file", type=str, default="/groups/sernam/datasets/coco/coco_2014val_caption_subset1000.json")
    parser.add_argument("--conv-mode", type=str, default="vicuna_v1_1")
    parser.add_argument("--temperature", type=float, default=0)
    parser.add_argument("--top_p", type=int, default=None)
    parser.add_argument("--prompt", type=str, default="generate a short caption for this image")
